  ...
  useEffect(() => {
    if (containerRef.current && window.MathJax) {
      window.MathJax.typesetPromise?.([containerRef.current])
        .catch(err => console.log('MathJax:', err));
    }
  }, []);

  return (
    <div className="review-text-content" ref={containerRef}>
...
const htmlContent = `<article>
            ...
            <p class="center">Let \(I\) be an input image of size \(P \times Q\), \(K\) be the convolution kernel of dimensions \(M \times N\) and the coordinates of the output feature map be denoted \(x\) and \(y\). <sup>[2]</p> <p>2D convolution is described as,</p>
            ...
            <p>The 2D Discrete Fourier Transform (DFT) of an image \(I\) with frequency domain coordinates \(u\) and \(v\) can be described as,<sup>[3]</p> <p></p>
            ...
            <p>To resize an image, we require interpolation to estimate the pixel values at new positions. We often use nearest neighbor, bilinear interpolation, or bicubic interpolation.<sup>[4]</p> <p></p>
